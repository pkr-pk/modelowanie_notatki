...
$$L(\lambda_1, \lambda_2, \dots, \lambda_k) = \lambda_1^{s_1} \left\{ \prod_{j=2}^k \left[ \lambda_j \prod_{i=1}^{j-1}(1 - \lambda_i) \right]^{s_j} \right\} \prod_{j=1}^k \left[ \prod_{i=1}^j(1 - \lambda_i) \right]^{b_j}$$
...
$$ L(\lambda_1, \lambda_2, \dots, \lambda_k) = \left( \prod_{j=1}^k \lambda_j^{s_j} \right) (1-\lambda_k)^{b_k} \prod_{i=1}^{k-1} (1-\lambda_i)^{b_i + \sum_{m=i+1}^k (s_m+b_m)} $$
...
$$ L(\lambda_1, \lambda_2, \dots, \lambda_k) = \left( \prod_{j=1}^k \lambda_j^{s_j} \right) (1-\lambda_k)^{r_k-s_k} \prod_{i=1}^{k-1} (1-\lambda_i)^{r_i-s_i} $$
...
Dla $y = y_k$, **estymator Kaplana-Meiera** [66] $S_n(y)$ dla $S(y)$ uzyskuje się przez zastąpienie $\lambda_j$ przez $\hat{\lambda}_j = s_j/r_j$ wszędzie, gdzie się pojawia. Zauważając, że $S(y) = S(y_j)$ dla $y_j \le y < y_{j+1}$, wynika z tego, że
$$ S_n(y) = \begin{cases} 1, & y < y_1, \\ \prod_{i=1}^j (1-\hat{\lambda}_i) = \prod_{i=1}^j (1 - s_i/r_i), & y_j \le y < y_{j+1}, \quad j=1, 2, \dots, k-1, \\ \prod_{i=1}^k (1-\hat{\lambda}_i) = \prod_{i=1}^k (1 - s_i/r_i), & y_k \le y < y_{max}. \end{cases} $$
...
1.  **Korekta ogona Efrona** [31] zakłada, że $S_n(y) = 0$ dla $y \ge y_{max}$.
2.  **Klein i Moeschberger** [70, str. 118] zakładają, że $S_n(y) = S_n(y_k)$ dla $y_k \le y < \gamma$ i $S_n(y) = 0$ dla $y \ge \gamma$, gdzie $\gamma > y_{max}$ jest prawdopodobnym górnym limitem dla podstawowej zmiennej losowej. Na przykład, w badaniu śmiertelności ludzkiej, limit ten może wynosić 120 lat.
3.  **Wykładnicza korekta ogona Browna, Hollandera i Korwara** [18] zakłada, że $S_n(y_{max}) = S_n(y_k)$ oraz że $S_n(y) = e^{-\hat{\beta}y}$ dla $y \ge y_{max}$. Przy $y = y_{max}$, $\hat{\beta} = -\ln S_n(y_k)/y_{max}$, a zatem
...
Alternatywą dla estymatora Kaplana-Meiera jest czasami używany **estymator Nelsona-Åalena** [1], [93]. Aby go zmotywować, zauważmy, że jeśli $S(y)$ jest funkcją przeżycia rozkładu ciągłego z intensywnością awarii $h(y)$, to $-\ln S(y) = H(y) = \int_0^y h(t)dt$ nazywa się **skumulowaną funkcją intensywności awarii**. Dyskretny odpowiednik, w obecnym kontekście, jest dany przez $\sum_{i|y_i \le y} \lambda_i$, co intuicyjnie można oszacować, zastępując $\lambda_i$ przez jej estymator $\hat{\lambda}_i = s_i/r_i$. Estymator Nelsona-Åalena dla $H(y)$ jest więc zdefiniowany dla $y < y_{max}$ jako
$$ \hat{H}(y) = \begin{cases} 0, & y < y_1, \\ \sum_{i=1}^j \hat{\lambda}_i = \sum_{i=1}^j s_i/r_i, & y_j \le y < y_{j+1}, \quad j=1, 2, \dots, k-1, \\ \sum_{i=1}^k \hat{\lambda}_i = \sum_{i=1}^k s_i/r_i, & y_k \le y < y_{max}. \end{cases} $$
Oznacza to, że $\hat{H}(y) = \sum_{i|y_i \le y} \hat{\lambda}_i$ dla $y < y_{max}$, a estymator Nelsona-Åalena funkcji przeżycia to $\hat{S}(y) = \exp(-\hat{H}(y))$. Notacja pod sumą wskazuje, że wartości $\hat{\lambda}_i$ powinny być uwzględniane tylko wtedy, gdy $y_i \le y$. Dla $y \ge y_{max}$ sytuacja jest podobna do tej z estymatorem Kaplana-Meiera, w tym sensie, że należy zastosować korektę ogona omówionego wcześniej typu. Zauważmy, że w przeciwieństwie do estymatora Kaplana-Meiera, $\hat{S}(y_k) > 0$, więc korekta ogona jest zawsze potrzebna.
...
co jest funkcją $\hat{\lambda}_i$. Zatem, aby oszacować wariancję $S_n(y)$, najpierw potrzebujemy macierzy kowariancji $(\hat{\lambda}_1, \hat{\lambda}_2, \dots, \hat{\lambda}_k)$. Szacujemy ją z "funkcji wiarygodności", używając standardowych metod wiarygodności. Ostatecznie, estymata wariancji jest dana przez
$$ \hat{Var}[S_n(y)] = [S_n(y)]^2 \sum_{i|y_i \le y} \frac{s_i}{r_i(r_i-s_i)}. \quad(14.2) $$
Równanie (14.2) jest znane jako **aproksymacja Greenwooda** wariancji $S_n(y)$ i wiadomo, że często zaniża prawdziwą wariancję.

Przechodząc teraz do estymatora Nelsona-Åalena, zauważamy, że $\hat{H}(y) = \sum_{i|y_i \le y} \hat{\lambda}_i$, i to samo rozumowanie co dla Kaplana-Meiera implikuje, że $Var[\hat{H}(y)] \approx \sum_{i|y_i \le y} Var(\hat{\lambda}_i)$, dając estymatę
$$ \hat{Var}[\hat{H}(y)] = \sum_{i|y_i \le y} \frac{s_i(r_i-s_i)}{r_i^3}, \quad(14.3) $$
nazywa się to **estymatą Kleina**.
...
$$ \hat{Var}[S_{20}(2)] = [S_{20}(2)]^2\left[\frac{1}{20(19)} + \frac{1}{19(18)}\right] = (0.90)^2(0.00556) = 0.0045, $$
$$ \hat{Var}[S_{20}(9)] = [S_{20}(9)]^2\left[\frac{1}{20(19)} + \dots + \frac{4}{8(4)}\right] = (0.267)^2(0.17890) = 0.01271, $$
a dla estymatora Nelsona-Åalena,
$$ \hat{Var}[\hat{S}(2)] = [\hat{S}(2)]^2\left[\frac{1(19)}{20^3} + \frac{1(18)}{19^3}\right] = (0.902)^2(0.00500) = 0.00407. $$
...
Oczywiście, przedziały ufności dla $S(2)$ są niezadowalające, ponieważ oba zawierają wartości większe niż 1.

Alternatywne podejście można skonstruować w następujący sposób. Niech $Y = \ln[-\ln S_n(y)]$. Używając metody delta, wariancję $Y$ można przybliżyć. Następnie, przybliżony 95% przedział ufności dla $\theta = \ln[-\ln S(y)]$ jest
$\ln[-\ln S_n(y)] \pm 1.96\frac{\sqrt{\hat{Var}[S_n(y)]}}{S_n(y) \ln S_n(y)}$.
Ponieważ $S(y) = \exp(-e^Y)$, wyliczenie każdego końca tego wzoru daje przedział ufności dla $S(y)$. Ten przedział zawsze będzie w zakresie 0-1 i jest określany jako **przekształcony logarytmicznie przedział ufności**.
...
W tym przypadku, $S_{20}(2) = 0.9$, a $U = \exp\left[1.96(0.06708)/(0.90 \ln 0.90)\right] = 0.24994$. Dolna granica to $(0.90)^{1/U} = 0.65604$, a górna granica to $(0.90)^U = 0.97401$.